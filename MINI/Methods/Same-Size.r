...
# Kmeans++ initiation
# D(x) denotes the shortest distance from a data point to the closest centre have been chosen.
# 1. Take one centre c_1, chosen uniformly at random from X.
# 2. Take a new centre c_i, choosing x from X with probability D(x)^2 / sum_(x in X) (D(x)^2).
# 3. Repeat step 2, until we have taken k centres altogether.
# 4. Proceed.

KmeansppInital <-function (x, k) {
  data.num = nrow(x)  # points
  centers = sample(1:data.num,1)  # randomly select from points
  while (length(centers) < k) {
    dist = apply(x, 1, function(i) {
      #distance to the nearest center
      min(sapply(centers, function(cid) {
        GetSingleDist(i, x[cid,]) ^ 2  # pairwise distance
      }))
    })
    #cat(dist/sum(dist))
    #cat("\n")
...
SameSizeKmeans <- function(x, k, max.iter = 100) {
  data.num = nrow(x)  # points
  cluster.size = ceiling(data.num / k)  # cluster size
  centers.idx = KmeansppInital(x, k)  # initiation with kmeans++
...
x <- read.csv("SPCA_order.csv", header = FALSE)
X <- as.matrix(x)
