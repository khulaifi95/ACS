...
X = load('/home/kevinxu95/ACS/MINI/SPCA_order.csv');  % 500 points, k=15, d=2
...
number_of_iterations_distribution = zeros(10,1);

for repeats = 1:10      % 1:100
...
while ((partition_changed)&&(kmeans_iteration_number<10))% kmeans iterations
...
##%plot(C(:,1),C(:,2),'gO');
##%hold on
##plot(X(find(partition==1),1:2),'r+');
##if k>1
##    hold on
##    plot(X(find(partition==2),1:2),'b0');
##    #plot(X(find(partition==2),1),X(find(partition==2),2),'bO');
##end
##if k>2
##    hold on
##    plot(X(find(partition==3),1:2),'r.');
##    #plot(X(find(partition==3),1),X(find(partition==3),2),'r.');
##end
##if k>3
##    hold on
##    plot(X(find(partition==4),1:2),'b.');
